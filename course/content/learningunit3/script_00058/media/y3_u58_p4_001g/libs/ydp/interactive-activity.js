(function (wnd) {
	...
	var svgimgZoom = document.createElementNS('http://www.w3.org/2000/svg', 'image');
	var svgimgBig = document.createElementNS('http://www.w3.org/2000/svg', 'image');
		...
		try {
			...
			if (!anim_startBtn_block) {
				setTimeout(function () {
					...
					setTimeout(function () {
						currentSound = sounds[questions[0].audio];
						// currentSound.play();
					}, 10);
				}, 100);
			}
			...
			$('#anim-audioBtn').on(eDown, function (e) {
				...
				if (!anim_audioBtn_block && !isPlaying) {
					...
					isPlaying = true;
					setTimeout(function () {
						currentSound = sounds[questions[currentQuestion - 1].audio];
						currentSound.play();
					}, 10);
	...
	function openBigImage() {
		if (myLightBox) {
			...
			myLightBox.changeImage(currentQuestion - 1);
	...
	function imageAdd() {
		if (!imageAddCheck) {
			imageAddCheck = true;
			imageChange(currentQuestion);
			svgimg.setAttributeNS(null, 'id', 'svgimg');
			...
			if (questions[currentQuestion].imageBig.indexOf('.jpg') !== -1 || questions[currentQuestion].imageBig.indexOf('.png') !== -1) {
				svgimgZoom.setAttributeNS('http://www.w3.org/1999/xlink', 'href', 'images/zoom.png');
				svgimgZoom.setAttributeNS(null, 'id', 'svgimgZoom');
				svgimgZoom.setAttributeNS(null, 'x', posX + width - 64);
				svgimgZoom.setAttributeNS(null, 'y', posY + height - 64);
				svgimgZoom.setAttributeNS(null, 'width', 64);
				svgimgZoom.setAttributeNS(null, 'height', 64);
				svgimgZoom.setAttributeNS(null, 'display', 'inline');
				$('#interactive-layer').append(svgimgZoom);

				$('#svgimg').on(eDown, function (e) {
					e.preventDefault();
					//clickImg();
					openBigImage();
				});

				$('#svgimgZoom').on(eDown, function (e) {
					e.preventDefault();
					//clickImg();
					openBigImage();
				});
	...
	function clickImg() {
		svgimgBig.setAttributeNS('http://www.w3.org/1999/xlink', 'href', questions[currentQuestion - 1].imageBig);
		svgimgBig.setAttributeNS(null, 'id', 'svgimgBig');
		svgimgBig.setAttributeNS(null, 'x', posXBig);
		svgimgBig.setAttributeNS(null, 'y', posYBig);
		svgimgBig.setAttributeNS(null, 'width', widthBig);
		svgimgBig.setAttributeNS(null, 'height', heightBig);
		svgimgBig.setAttributeNS(null, 'display', 'inline');
		$('#svg_0').append(svgimgBig);
		$('#svg_0').css('z-index', '5');
		$('#svg_0').on(eDown, function (e) {
			$('#svg_0').css('z-index', '0');
		});
	}

	function imageChange(currentQ) {
		svgimg.setAttributeNS('http://www.w3.org/1999/xlink', 'href', questions[currentQ].image);
		setTimeout(function () {
			svgimg.setAttributeNS(null, 'display', 'inline');
		}, 100);
	}
	...
	function letterPress(ID) {
		...
		if (!questionObj.missing0.length) {
			...
			isPlaying = true;
			setTimeout(function () {
				currentSound = sounds['ok'];
				currentSound.play();
			}, 10);
		} else {
			isPlaying = true;
			setTimeout(function () {
				currentSound = sounds['wrong'];
				currentSound.play();
			}, 10);
			currentChance++;
			showFeedback();
	...
	function reShowFeedback() {
		...
		$('#anim-color_spots').attr('display', 'none');
	...
	p.loadState = function (obj) {
		...
		if (questions[currentQuestion - 1].image.indexOf('.jpg') !== -1 || questions[currentQuestion - 1].image.indexOf('.png') !== -1) {
			svgimg.setAttributeNS(null, 'display', 'none');
			imageAdd();
			imageChange(currentQuestion - 1);
		}
